...
def parse_config_file(file_name):
    ...
    for line in open(file_name).readlines():
        ...
        if line.startswith("!"):
            if line.lower()[1:].strip().startswith("force env: "):
                forced_env = line.lower().split(":", maxsplit=1)[1].strip()
            continue
        ...
        if len(tree) > 1:  # if there's something with more words
            if tree[0] in ["test", "dev", "prod"]:  # check if it's an env
                if tree[0] == configuration.get_current_env(forced_env):  # check if it's current env
